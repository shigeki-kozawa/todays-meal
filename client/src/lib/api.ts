        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            ...
            if (data.type === 'conversationId') {
              ...
              // #region agent log
              fetch('http://127.0.0.1:7243/ingest/6239307f-1694-4acf-8801-2adc029deba1',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'api.ts:recipe-received',message:'Client received recipe',data:{recipeName:data.data.name,hasImageUrl:!!data.data.imageUrl,imageUrl:data.data.imageUrl},timestamp:Date.now(),sessionId:'debug-session',runId:'run2-image',hypothesisId:'B,C'})}).catch(()=>{});
              // #endregion
              onRecipe(data.data)
